...
class OffCampusPassListStudent extends StatefulWidget {
  const OffCampusPassListStudent({super.key});

  @override
  State<OffCampusPassListStudent> createState() => _OffCampusPassListStudentState();
}

// 1. TickerProviderStateMixin 추가
class _OffCampusPassListStudentState extends State<OffCampusPassListStudent> with TickerProviderStateMixin {
      ...
      body: Padding(
        ...
        child: Obx((){
          if (offCampusPassController.isLoadingOffCampusPassData.value) {
            ...
            final dataList = offCampusPassController.offCampusPassData.where((data) =>
            data.grade == Constants.currentUser.grade && data.classNum == Constants.currentUser.classNum && data.number == Constants.currentUser.number).toList();
            return ListView.builder(
              itemCount: dataList.length,
              itemBuilder: (context, index) {
                final data = dataList[index];
                return data.endTime!.isAfter(DateTime.now()) ?
                GestureDetector( // Card 위젯으로 변경하지 않고 기존 구조 유지
                    onTap: () async {
